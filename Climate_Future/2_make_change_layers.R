...
s3_files <- s3_ls(s3_out)

# Ignore the modelmeans files
s3_files <- s3_files[!grepl('modelmeans', s3_files$file), ]
...
foreach(this_variable=unique(s3_files$variable)) %:%
    foreach(this_season=unique(s3_files$variable)) %do% {
    ...
    base_m <- foreach(base_file=base_files$file, .combine=stack) %dopar% {
        ...
        system2('aws', args=c('s3', 'cp', paste0(s3_out, base_file), temp_file))
        ...
        foreach(this_period=periods) %dopar% {
            scen_files <- filter(s3_files,
                ...
                scen_m <- foreach(scen_file=scen_files$file, .combine=stack) %do% {
                    temp_file <- tempfile(fileext='.tif')
                    system2('aws', args=c('s3', 'cp', paste0(s3_out, scen_file), temp_file))
                    model_data <- brick(temp_file)
                    mod_mean <- mean(model_data)
                    unlink(temp_file)
                    return(mod_mean)
